...
Brain = (function() {
  function Brain(master) {
    ...
    this.caseSensitive = master._caseSensitive;
  ...
  Brain.prototype.processTags = function(user, msg, reply, st, bst, step, scope) {
    var botstars, content, data, formats, giveup, i, insert, j, k, l, len, m, match, name, output, parts, random, ref, ref1, replace, result, stars, subreply, tag, target, text, type, value;
    stars = [""];
  ...
  Brain.prototype.processCalls = function(giveup, match, reply, scope) {
    var args, lang, obj, output, parts, text;
    if (!match) {
      return reply;
    }
    giveup++;
  ...
  Brain.prototype.asyncCallReply = function(outputP, reply, match, giveup, scope) {
    return outputP.then((function(_this) {
      return function(output) {
        reply = reply.replace(new RegExp("<call>" + utils.quotemeta(match[1]) + "</call>", "i"), output);
        match = reply.match(/<call>(.+?)<\/call>/i);
        return _this.processCalls(giveup, match, reply, scope);
      };
    })(this));
  };
